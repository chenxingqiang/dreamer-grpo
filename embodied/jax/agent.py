...
try:
    ...
    # Handle the "_default_backend is not None" assertion error
    ...
    for module in list(sys.modules.keys()):
        if module.startswith('jax') or module == 'xla_bridge':
...
class Agent(embodied.Agent):
  ...
  def __init__(self, model, obs_space, act_space, config, jaxcfg):
    ...
    # Try to get JAX devices with robust error handling
    max_attempts = 3
    for attempt in range(max_attempts):
        try:
            ...
            # Re-import JAX modules
            importlib.invalidate_caches()
            import jax
            import jax.experimental.multihost_utils
            import jax.numpy as jnp
            
            # Last attempt - if this fails, let it raise
